...
```
Цепочка начинается с того, что происходит событие, связанное с каким-либо визуальным элементом на экране. Например, пользователь кликает на карточку товара в галерее. а) Действие приложения переходит в класс Card (класс представления, View). б) Обработчик клика, привязанный к карточке товара в классе Card, вызывает метод emit посредника (класса EventEmitter, Present), который инициирует обработчик события 'card:select', передавая ему данные — объект товара, по которому был клик. в) Обработчик события 'card:select', получив данные, вызывает метод модели (класс AppData, Model) setPreview, который снова вызывает метод emit посредника, инициируя обработчик события 'preview:changed' и передавая ему данные — объект конкретного товара. г) Обработчик события 'preview:changed' передаёт выполнение в класс Modal (снова View), который с помощью своих методов отображает модальное окно, используя данные о выбранном товаре.
```

## Данные и типы данных используемые в приложении
Тип, описывающий возможные категории карточки
```
CardCategory = 'другое' | 'софт-скил' | 'дополнительное' | 'кнопка' | 'хард-скил';
```
...
##### Свойства:
- `catalog: ICardItem[]` - данные о товаре.
- `basket: ICardItem[] = []` - данные о товаре в корзине.
- `order: IOrder = {payment: '',address: '',email: '',phone: '',total: null,id: []}` - данные о заказе.
...
- `orderReset(): void` – очищает все сохраненные данные заказа пользователя.
...
- `set address(value: string)` - Свойство для установки значения адреса доставки.
...
- `disableButton(value: number | null): void` - устанавливает класс disabled, если цена товара null;
